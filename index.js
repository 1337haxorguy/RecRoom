...
app.post('/api/getCoverImageMovie', async (req, res) => {
    const input = req.body.input;

    // Call the pullShow function and handle the response
    const showNames = await getCoverImage(input, "movie");
    console.log(showNames)
    return res.json(showNames);
});

app.post('/api/getCoverImageTV', async (req, res) => {
    const input = req.body.input;

    // Call the pullShow function and handle the response
    const showNames = await getCoverImage(input, "tv");
    return res.json(showNames);
});
